...
import com.zbcn.common.model.bo.ParameterInvalidItem;
...
public class ParameterInvalidItemHelper {
    ...
    public static <T> List<ParameterInvalidItem> convertCVSetObjectToParameterInvalidItemList(Set<ConstraintViolation<T>> cvset) {
        List<ParameterInvalidItem> parameterInvalidItemList = getParameterInvalidItems(cvset);
        if (parameterInvalidItemList == null) return null;

        return parameterInvalidItemList;
    }

    private static <T> List<ParameterInvalidItem> getParameterInvalidItems(Set<ConstraintViolation<T>> cvset) {
        if (CollectionUtils.isEmpty(cvset)) {
            return null;
        }

        List<ParameterInvalidItem> parameterInvalidItemList = Lists.newArrayList();
        List<ParameterInvalidItem> parameterInvalidItemList1 = parameterInvalidItemList;
        for (ConstraintViolation<?> cv : cvset) {
            ...
            parameterInvalidItemList1.add(parameterInvalidItem);
        }
        return parameterInvalidItemList1;
    }
